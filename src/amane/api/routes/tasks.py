...
async def _task_titles(repo: RepoDep, tasks: Sequence[Task]) -> dict[int, str | None]:
    """scrape→番号, actor_scrape→演员名, refresh/organize/archive→库名. 批量查库, 避免列表 N+1."""
    actor_ids = {
        int(p["actor_id"])
        for t in tasks
    ...
    library_ids = {
        int(p["library_id"])
        for t in tasks
        if t.type in (TaskType.REFRESH, TaskType.ORGANIZE, TaskType.ARCHIVE)
    ...
    for t in tasks:
        ...
        elif t.type in (TaskType.REFRESH, TaskType.ORGANIZE, TaskType.ARCHIVE):
