...
可配置一组正则规则, 对文件名 (含扩展名) 进行匹配, 命中文件不入库且归档时移入 `.amane_trash`,此目录在扫描与监控中始终忽略.
...
设置最小视频大小 (默认关闭). 小于此值的文件不会被当作正片入库, 归档时移入 `.amane_trash`:
...
- 目前仅支持手动触发
- 库页面的整理会先提交归档任务 (黑名单与过小视频移入 `.amane_trash`), 再提交整理任务
- 文件表勾选后的批量整理只整理所选索引, 不归档
- 整理时会自动下载缺失的资源 (如海报)
