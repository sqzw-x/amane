...
桌面版的冷配置按以下顺序取值: 系统环境变量 > 数据目录旁的设置文件 > 内置默认值. 设置文件位于 macOS 的 `~/Library/Application Support/Amane/desktop.env` 与 Windows 的 `%LOCALAPPDATA%\Amane\desktop.env`, 首次启动自动生成注释模板; 菜单栏 / 托盘菜单的「打开设置文件」可直接编辑. 修改后经菜单「重启服务器」生效.

每行一个 `KEY=VALUE`, 以 `#` 开头的行为注释. 可写入的键:

| 键 | 默认值 | 说明 |
| ---------- | -------- | ------ |
| `AMANE_DATA_DIR` | 系统应用数据目录 | 数据库、资源与配置的存放位置; 修改后不会迁移已有数据 |
| `AMANE_LOG_DIR` | 数据目录下的 `logs` | 日志输出位置 |
| `AMANE_PORT` | `18000` | API 监听端口, 与其它服务冲突时修改 |
| `AMANE_HOST` | `127.0.0.1` | 监听地址; 改为 `0.0.0.0` 会让同网段可访问 |
| `AMANE_SAFE_DIRS` | `ALLOW_ALL` | 文件浏览器与库路径的边界目录, 逗号分隔 |
| `AMANE_TOKEN` | (自动生成) | API 访问令牌; `off` 关闭鉴权 |
