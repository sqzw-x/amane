...
单一源 `assets/logo.svg` (蓝色渐变圆角徽标 + 播放字腔字标 A). 修改后执行 `just icons`, 提交衍生文件:
...
- **应用进程**: `macapp/Sources/Amane` → `Contents/MacOS/Amane` (CFBundleExecutable). Launch Services 登记为 `com.github.sqzw-x.amane` (必须是 NSApplication, 第二次打开才走系统单实例). 本进程设置环境、监督 Python、启动与回收菜单栏. 服务退出码 **0 / 130 / 143** 结束 App; **3** 立刻再次启动 Python (UI 继续活着); **126 / 127** exec 失败退出; 其它退避 2s. TERM 时先停两个子进程. Info.plist: `LSUIElement` + `LSMultipleInstancesProhibited`. `AMANE_SUPERVISED=1`.
...
- **应用进程**: `winapp/` → `Amane.exe`. 命名 Mutex `Local\com.github.sqzw-x.amane` 单实例. 隐藏窗口泵消息 + 托盘; 后台线程监督 Python. Python 放入 Job Object (`KILL_ON_JOB_CLOSE`), 任务管理器杀掉壳时服务一起结束. 退出码 **0 / 130 / 143 / 0xC000013A** (Windows Ctrl+C) 结束 App; **3** 立刻再次启动 Python (托盘还在); `Process.Start` 失败结束 App; 其它退避 2s. `AMANE_SUPERVISED=1`.
- **服务进程**: PyInstaller onedir `onedir/Amane.Server.exe`, 由壳 `CreateNoWindow` 启动.

Windows 托盘不能放入 Python 进程: `exit 3` 会移除图标, PyInstaller 身份更容易触发 Defender. 壳与托盘同进程: 托盘只做 HTTP 轮询与菜单, 不另拆 exe.
...
| 退出 | 菜单「退出」停壳; 壳先停 Python (就绪时经由同一条 `POST /api/system/restart` 优雅停机, 因 stopping 不再再次启动; 否则 Kill), 再卸托盘 |
...
Windows 无独立 UI 进程, 无这组 argv. `AMANE_UI_ONLY=1` 只开托盘、不启动 Python (`just windows-bar`), 对已有服务轮询; 「退出」只关托盘.

## 生命周期

**macOS**

- **打开 Amane.app** → Launch Services 启动应用进程 → 设置环境 → spawn Python → 等 token 文件 → spawn UI.
- **第二次打开** → Launch Services 按 `com.github.sqzw-x.amane` 拦截.
- **UI 意外退出** → 应用进程退避后再次启动 UI (Python 不受影响).
- **菜单「退出 Amane」** → SIGTERM 应用进程 → 停 Python + UI.
- **菜单「重启服务器」** → `POST /api/system/restart` → Python `exit 3` → 应用进程立刻再次启动 Python; UI 还在, 轮询会重新连上.
- **Python 崩溃** (其它非 0) → 应用进程退避后再次启动; UI 仍在.
- **对 onedir/Amane Force Quit (SIGKILL)** → 当成崩溃再次启动. 停 App 用菜单或结束应用进程.

**Windows**

- **打开 Amane.exe** → Mutex; 已有实例则立刻退出 → 设置环境 → 托盘 + spawn Python; token 文件就绪后轮询带鉴权.
- **第二次打开** → Mutex, 退出.
- **菜单「退出 Amane」** → stopping, 停 Python, 卸托盘, 结束消息循环.
- **菜单「重启服务器」** → 同 macOS: `exit 3`, 托盘不拆.
- **Python 崩溃** → 退避后再次启动; 托盘不拆.
- **任务管理器结束 Amane.exe** → Job Object 结束 Python.
...
- `AMANE_HOST=127.0.0.1` `AMANE_PORT=18000` (可覆盖; 绑定回环, 避免防火墙弹窗)
...
macOS: `scripts/build_macos_app.sh` (`just macos-app`). PyInstaller 打成 onedir 后再组装 `.app`. 入口与导入约束见 [architecture.md](architecture.md). 需要 Swift 工具链.

1. PyInstaller onedir (`scripts/pyinstaller_entry.py`)
...
Windows: `scripts/build_windows_app.ps1` (`just windows-app`). 必须在 Windows 上运行 (PyInstaller 与 Native AOT 都不能从 macOS 交叉). 需要 .NET 8 SDK + 能链 Native AOT 的 MSVC.
