...
ORGANIZE 复制到库路径的 poster 先按 `scraping.poster_ratio` 居中裁剪归一化宽高比 (只裁两侧, 不补边, 不放大; 与目标比差异小于 0.005 时不改写). Emby / Jellyfin 以固定宽高比渲染海报, 偏宽的图由客户端裁掉两侧, 叠在上角的角标会连同边距一并被切除. 归一化必须在叠角标之前执行.
