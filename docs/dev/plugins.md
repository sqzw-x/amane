...
- `probe`: 列出本源在这个条目上能提供的流, 一条流一项: 稳定标识 `key`、在来源内的展示名 `name`、媒体类型、是否可按字节寻址, 以及该条流自己的 `available` 与 `detail`. 一个来源可以给出多条流 (多个搜索结果、多个已入库文件), 顺序即列表顺序; 条目里没有一条可播时仍把候选列出来并逐条说明原因. 整个来源都没有东西可列时返回空元组, 或抛 `SourceError(NO_USABLE_METADATA, detail=中文原因)` 来说明「没有内容」的原因.
...
`key` 由插件声明并保证在同一来源与条目内稳定: 浏览器地址、解析结果缓存、HLS 分片 token 的归属都按它区分, 改动它等于让在播地址失效. 主机不解释 `key` 的含义, 也不核对它是否对应该条目的某个文件 —— 认不出来的 `key` 由插件自己拒绝 (`SourceError(NO_USABLE_METADATA)`, 如「所选文件不在该条目的索引中」); 只有进路径的形状 (`^[a-zA-Z0-9._-]{1,64}$`) 由主机校验. 列表每一行的展示名是 `来源名 · 流的展示名`, 因此 `name` 里不要重复来源名.

`probe` 的 `available=false` 只表示这一条流当下不可播 (文件为空、已从磁盘消失), 原因同样原样展示给用户, 且只写用户能据以行动的信息 (哪个文件怎么了), 不写完整路径与上游地址.
...
探测预算 2 秒, 覆盖该来源这一次 `probe` 的全部工作. 超时或抛错时该来源在列表里只剩一行不可用 (名字取 descriptor 的展示名), 以 `detail="探测超时"` / `"探测失败"` 呈现. 网络失败抛 `SourceError`. 空结果与 `NO_USABLE_METADATA` 走同一条探测负缓存, 原因跟着缓存保留; 其余 `SourceError` 归为「上游失败」并单独记负缓存. 列表结果按「来源 + 条目」缓存, 与用户选中的流无关. `probe` 内访问网络可能耗尽预算; 列出多个候选时预算不随候选数增长 —— 逐个访问上游的清单只能在 `resolve` 做.
