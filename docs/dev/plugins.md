...
- `probe` 的两种否定回答必须分清: 返回**空元组**表示「这个来源没有内容, 也没什么可解释的」, 列表里那一行不带原因, 若它是唯一来源则整块播放区不渲染; 抛 `SourceError(NO_USABLE_METADATA, detail=中文原因)` 表示「没有内容, 但原因值得告诉用户」, 列表以 `available=false` 与这条 `detail` 呈现一片红色提示.
...
`key` 由插件声明并保证在同一来源与条目内**稳定且唯一**: 浏览器地址、解析结果缓存、HLS 分片 token 的归属都按它区分, 改动它等于让在播地址失效, 重复则两条流共用同一个地址与缓存桶 (主机丢弃后一条并记日志). 形状是 `^[a-zA-Z0-9._-]{1,64}$`, 但整值 `.` 与 `..` 被拒绝 —— 这两个段会被浏览器与服务器归一化掉, 请求落到别的地址上. 主机不解释 `key` 的含义, 也不核对它是否对应该条目的某个文件 —— 认不出来的 `key` 由插件自己拒绝 (`SourceError(NO_USABLE_METADATA)`, 如「所选文件不在该条目的索引中」); 只有进路径的形状由主机校验. 列表每一行的展示名是 `来源名 · 流的展示名`, 因此 `name` 里不要重复来源名. 列表顺序就是 `query.files` 的顺序 (主机按入库顺序排列), 前端默认选中第一条可播的, 因此这个顺序对用户可见.

`probe` 的 `unavailable` 表示这一条流当下不可播 (文件为空、已从磁盘消失), 原因原样展示给用户, 且只写用户能据以行动的信息 (哪个文件怎么了), 不写完整路径与上游地址. 没有这个字段就是可播 —— 「列出来但没写原因」会让整块播放区消失, 比说明原因更差.
...
探测预算 2 秒, 覆盖该来源这一次 `probe` 的全部工作: 主机只保证这个总时长, **不保证预算与候选数无关** —— 列出 N 个候选就要在 2 秒内核对完 N 个, 逐个候选的 IO (含 `stat` 与读字幕) 必须由插件自己限制在预算内. 超时或抛错时该来源在列表里只剩一行不可用 (名字取 descriptor 的展示名), 以 `detail="探测超时"` / `"探测失败"` 呈现, 已经列出的候选不会留下半截. 网络失败抛 `SourceError`. 空结果与 `NO_USABLE_METADATA` 走同一条探测负缓存, 原因跟着缓存保留; 其余 `SourceError` 归为「上游失败」并单独记负缓存. 列表结果按「来源 + 条目」缓存, 与用户选中的流无关. 需要按候选逐个访问上游的清单只能在 `resolve` 做.
