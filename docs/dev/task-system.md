...
- **封面角标**: `watermark.enabled` 时 poster / thumb 副本按该 Metadata 下同库全部文件的聚合相位叠 PNG; 大小 / 四角见 Hot `watermark`. Resource 原图与 fanart 不修改. 用户 PNG 覆盖见 [data-model.md](data-model.md).
