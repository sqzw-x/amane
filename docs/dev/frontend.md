...
画面上的右键菜单给出两个复制入口: 当前时间戳, 以及在现有地址上换掉 `t` 参数的分享地址 (其余参数原样保留, 落点在 0 秒时删掉该参数). 时间戳格式与评论里可点击的时间戳同形, 复制出来可直接粘回评论. 复制的秒数在点按条目时读取, 不是展开菜单时的那一秒 —— 展开期间画面仍在播放, 因此不订阅 `timeupdate`. 菜单展开期间, 菜单之外的任何点击都先收起菜单, 且**那一次点击不切换播放/暂停** —— 画面点击的单击判定在这期间被挂起 (主流的右键菜单都如此); 其余点击照常生效. 菜单必须渲染在控制器内部: 容器带 `overflow: hidden`, 挂到控制器外会被裁掉, 全屏时也只有控制器的子树可见 (Mantine 的 Portal 挂在 `document.body`). 右键不走 React 的合成事件 —— `contextmenu` 不派发到自定义元素上的 React 监听 —— 监听挂在控制器上. 判定与控制条一致: 只有画面与控制器自身响应, 控制条与菜单保留浏览器原生右键菜单; 不可寻址的流两项置灰.
