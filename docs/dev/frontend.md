...
画面上的右键菜单给出两个复制入口: 当前时间戳, 以及在现有地址上换掉 `t` 参数的分享地址 (其余参数原样保留, 落点在 0 秒时删掉该参数). 时间戳格式与评论里可点击的时间戳同形, 复制出来可直接粘回评论. 菜单必须渲染在控制器内部: 容器带 `overflow: hidden`, 挂到控制器外会被裁掉, 全屏时也只有控制器的子树可见 (Mantine 的 Portal 挂在 `document.body`). 播放位置在展开时读取一次, 不订阅 `timeupdate`. 判定与控制条一致: 只有画面与控制器自身响应, 控制条与菜单保留浏览器原生右键菜单; 不可寻址的流两项置灰.
