...
评论区 (`components/media/comment-section.tsx`) 的排序与编辑态只存在组件内, 不写入地址栏. 排序键是编辑时间 `updated_at`; 楼号按创建顺序固定, 不随排序翻转或编辑变化, 否则同一条评论的序号会跳. 时间显示相对时间 (一分钟内为「刚刚」), 编辑过的在同一行补出「（已编辑 …）」. 正文用 `pre-wrap`: 折叠换行会让多行评论读成一段. 时间戳用浅色胶囊而不是下划线 —— 下划线在中文正文里与着重号混淆, 且 `--mantine-primary-color-filled` 在深色卡片上对比不足. 卡片底色比其它卡片高一档 (`light-dark(gray-1, dark-5)`): 卡片默认底色与页面底色只差一档, 整块读不出来. 删除经 `confirm`; 编辑在行内展开, Esc 取消, Ctrl/Cmd + Enter 保存. 行内操作紧跟在时间之后 (不放行尾 —— 宽屏下指针要跨越整行才能点到), 只在指针悬浮该条或焦点进入该条时出现, 且只在 `@media (hover: hover)` 内隐藏 —— 触摸设备没有悬浮态, 隐藏会让按钮永远点不到; 用 `opacity` 而非 `visibility` 隐藏, 悬浮时不会引起行内重排. 评论与关联文件在 `lg`(1200px) 以上并排各占一半, 以下落回上下叠放; 断点不能降到 `md`, 再窄文件路径会明显截断. 两栏各自保持自然高度 (`align-items: start`), 拉平只会把空白挪进较短的一栏.
