...
影片详情在封面预告片之外播放正片. 可播放源由 `GET /api/playback/sources` 探测, 列表为空则不展示播放入口. `video` 的媒体地址只使用列表给出的本机路径; 多于一个源时提供选择. 码流与字幕均为同源 `/api/playback/...`, 原生 `<video>` 不设置 `crossOrigin` (凭据模式与 `Access-Control-Allow-Origin: *` 不能同时成立). `video/*` 使用原生控件; HLS (`mpegurl`) 在 Safari 等可原生播放的浏览器采用 `<video src>`, 其它浏览器用 `hls.js` 且 XHR `withCredentials`. 列表给出的 WebVTT 渲染为 `<track>`. 不允许把上游 URL 或密钥写入前端状态. 播放失败展示主机返回的中文 `detail`.
