...
影片详情: 用户标签与刮削标签分栏; 加减菜单一次提交多名 — 标签挂载只有 `POST /api/metadata/batch/user-tags` 一个入口, 影片与标签两个维度都是集合 (单条写入即 1×1), 未知标签 id 返回 404; 新建标签同样只经 `POST /api/facets/user_tag`, 已存在的名称复用; 演员详情有同样的标签分栏, 经 `/api/actors/batch/user-tags`. 演员浏览经由 `/api/actors`, 身份治理仍调用 `/api/facets/actor`, 筛选字段的单一事实源是 `lib/actors/browse.ts`. 演员详情的出演作品经 `/api/metadata` 取数, 排序项与片库同源 (`lib/media/browse.ts` 的 `METADATA_SORT_OPTIONS`); 该页没有其它导航态, 排序记忆落在 ui store 的 `actorWorksSort`, 不写地址栏, 读取经 `metadataSortPreferenceSchema` 校验.
