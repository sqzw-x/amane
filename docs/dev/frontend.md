...
影片详情在封面预告片之外播放正片. 可播放源由 `GET /api/playback/sources` 探测, 列表为空则不展示播放入口. `video` 的 `src` 只使用列表给出的本机码流路径; 多于一个源时提供选择. 仅 `video/*` 使用原生控件. 不允许把上游 URL 或密钥写入前端状态. 播放失败展示主机返回的中文 `detail`.
