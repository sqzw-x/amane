...
- `/playback/sources` 须注册在 `/{source_id}` 之前. 列表含当前条目已启用的全部源 (禁用与未安装的源不出现), 不可用项以 `available=false` 与 `detail` 说明原因, 名字取插件 descriptor 的展示名 (`detail` 缺失表示来源没给出原因); HLS 项的 `href` 指向 `index.m3u8`. 列表 ETag 支持 `If-None-Match` 的多值与弱校验前缀, 304 仍带 `Cache-Control`. 码流 `HEAD`/`GET` 支持单段 Range. 清单改写后的分片经由 `hls/{token}`; 字幕经由 `subtitles/{track_id}` (`text/vtt`). `media_file_id` 必须属于路径中的 `metadata_id`, 否则 404. 超长 `source_id` 与未启用、未安装同样 404. 上游失败与畸形上游 URL 均为 502, 上游 416 原样返回. 码流 `upstream` 拒绝播放列表类型. 插件声明的 `file` 目标不属于该条目索引时也是 502. 契约见 [plugins.md](plugins.md).
