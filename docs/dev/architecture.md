...
| `library/` | 库文件规则与分类 (`LibraryScan.classify`) + CloudDrive 虚拟路径规范 | 扩展名 / 预告片与黑名单正则 / `.amane_trash` / 体积阈值 / `cloud_path`; handlers 与 scheduler 共用, 不纳入任何一侧 |
| `scheduler/` | 队列消费 / cron / 文件监控 / CloudDrive webhook / RSS 发现 | 与 api 解耦, 通过 EventBus 上报; webhook 契约见 [watcher.md](watcher.md) |
...
- **PlaybackFactory 在网络栈与 safe_dirs 之后**: 内置本地源依赖库路径边界; 上游反代使用独立流式客户端, 不复用刮削 `HttpClient`. rebuild 时替换并关闭旧客户端. 契约见 [plugins.md](plugins.md) / [api.md](api.md).
