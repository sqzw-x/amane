...
- **path 投影列** — `MediaFile` 相位列从 path 用 `parse_file_info` 回填; autogenerate 会把 StrEnum 写成成员名 Enum (CENSORED), 必须改成存 **value** 的 String, 并丢掉 `ix_metadata_number` / `schedules.task_type` 噪声.
