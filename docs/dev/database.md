...
- **JSON 列内部结构** — `Metadata.raw` 等 blob 不在 autogenerate 视野里. 爬虫/聚合模型修改字段名或类型时, **结果列与 raw 快照是两份数据**; 只修改列定义不够, 必须另写 data revision 遍历 JSON (见 `c4f17334c3ea`). 站点级复用会把 raw 直接 `MediaMetadata(...)`, 旧 key 会被 Pydantic 静默丢掉. `actors` 允许字符串列表与 `{name, gender}` 对象列表并存, `MediaMetadata` 两种都接受, 已有 raw 行不要求改写.
...
schema_template.py 在进程首次调用时构建已迁移的 SQLite 模板并 vacuum, 后续 `copy_schema()` 拷贝该文件, 测当前 schema 而非迁移路径. 业务测试用法与例外见 [testing.md](testing.md).
